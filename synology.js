		...
		this.drawFolder = function (elems) {
			...
			if (files.length) {
				...
				files.forEach(function (element) {
					...
					item
						.find('.dlna_client-file__size')
						.text(add + Lampa.Utils.bytesToSize(element.size))
		...
		this.displayFolder = function () {
			...
			$.ajax({
				...
				success: function (response) {
					var filesAndDirectories = _this3.parseXmlResponse(
						response.documentElement.outerHTML
					)
				...
				for (var i = 0; i < node.childNodes.length; i++) {
					if (node.childNodes[i].nodeType === 1) {
						...
						for (var j = 0; j < node.childNodes[i].attributes.length; j++) {
							nodeInfo[node.childNodes[i].attributes[j].name] =
								node.childNodes[i].attributes[j].value
						}
					}
				}
				return nodeInfo
